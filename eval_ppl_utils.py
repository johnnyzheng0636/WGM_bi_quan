...
def gptamodel_eval(model, testenc, dev,  dataset: str, log_wandb: bool = False):
    print("Evaluating ...")
    ...
    #         local_emb = model.model.rotary_emb_local(
    #             inps[j].unsqueeze(0), 
    #             position_ids=pos_ids[j], 
    #             # seq_len=model.seqlen
    #         )
    #         global_emb = model.model.rotary_emb(
    #             inps[j].unsqueeze(0), 
    #             position_ids=pos_ids[j], 
    #             # seq_len=model.seqlen
    #         )
    ...
    #             inps[j].unsqueeze(0), 
    #             position_ids=pos_ids[j],
    #             position_embeddings_global=global_emb,
    ...
    # TODO
    # use gptqmodel to load the gptq model and get the correct logits
    from gptqmodel import GPTQModel
    model = GPTQModel.load(model)
    batch = testenc[:, (i * model.seqlen) : ((i + 1) * model.seqlen)].to(dev)
    model = model.to(dev)
    outs = model(batch)
    ...
    nlls = []
    for i in range(nsamples):
        hidden_states = outs[i].unsqueeze(0)
        if model.model.norm is not None:
...
@torch.no_grad()
def gemma3_4b_eval(model, testenc, dev,  dataset: str, log_wandb: bool = False):
    print("Evaluating ...")
    ...
    model.model.language_model.rotary_emb_local = model.model.language_model.rotary_emb_local.to(dev)
    ...
    if model.model.language_model.norm is not None:
...
@torch.no_grad()
def gemma3_1b_eval(model, testenc, dev,  dataset: str, log_wandb: bool = False):
    print("Evaluating ...")
